...
class StreamsTestBaseService(KafkaPathResolverMixin, JmxMixin, Service):
    ...
    def pids(self, node):
        try:
            pids = list(node.account.ssh_capture(f"cat {self.PID_FILE}", callback=str))
        ...
        if clean_shutdown:
            for pid in pids:
                wait_until(
                    ...
                    timeout_sec=120,
                    err_msg=f"Streams Test process on {str(node.account)} took too long to exit",
                )
    ...
    def restart(self):
        ...
        for node in self.nodes:
            self.logger.info(f"Restarting Kafka Streams on {str(node.account)}")
            self.stop_node(node)
    ...
    def abortThenRestart(self):
        ...
        for node in self.nodes:
            self.logger.info(f"Aborting Kafka Streams on {str(node.account)}")
    ...
    def start_cmd(self, node):
        ...
        self.logger.info(f"Executing streams cmd: {cmd}")
    ...
    def start_node(self, node):
        ...
        self.logger.info(f"Starting StreamsTest process on {str(node.account)}")
        with node.account.monitor_log(self.STDOUT_FILE) as monitor:
            node.account.ssh(self.start_cmd(node))
            monitor.wait_until(
                self.expectedMessage,
                timeout_sec=60,
                err_msg=f"Never saw message indicating StreamsTest finished startup on {str(node.account)}",
            )
...
class StreamsUpgradeTestJobRunnerService(StreamsTestBaseService):
    ...
    def start_cmd(self, node):
        ...
        if self.KAFKA_STREAMS_VERSION in [str(LATEST_0_10_0), str(LATEST_0_10_1)]:
